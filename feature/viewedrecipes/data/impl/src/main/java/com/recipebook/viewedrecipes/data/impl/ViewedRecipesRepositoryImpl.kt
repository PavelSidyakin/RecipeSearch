...
internal class ViewedRecipesRepositoryImpl @Inject constructor(
    ...
    override suspend fun requestViewedRecipes(): List<ViewedRecipeDetails> = withContext(Dispatchers.IO) {
        viewedRecipeDao.requestViewedRecipes().map { it.toViewedRecipeDetails() }
    }

    override suspend fun requestFavoriteViewedRecipes(): List<ViewedRecipeDetails> = withContext(Dispatchers.IO) {
